...
protected:
    ...
    static void test_task_handler_with_mutex(Allocator& allocator, uint8_t value);
};
...
TEST_F(AllocatorTest, Allocate) {
    ...
    ASSERT_EQ(allocator.allocate_with_atomic(), nullptr);
    ASSERT_EQ(allocator.allocate_with_atomic(), nullptr);
}
...
void AllocatorTest::test_task_handler_with_mutex(Allocator& allocator, uint8_t value) {
    uint32_t counter = 10000;
    std::array<void*, 2> blocks;
    std::array<uint8_t, block_size> sample;
...
TEST_F(AllocatorTest, Multithreading_with_mutex) {
    Allocator allocator;
    std::thread task1{test_task_handler_with_mutex, std::ref(allocator), 1};
    std::thread task2{test_task_handler_with_mutex, std::ref(allocator), 2};
    std::thread task3{test_task_handler_with_mutex, std::ref(allocator), 3};
    std::thread task4{test_task_handler_with_mutex, std::ref(allocator), 4};
